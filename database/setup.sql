...
CREATE TABLE IF NOT EXISTS matches (
    ...
    stage TEXT CHECK (
        stage IN ('group_stage', 'playoff', 'grand_final')
    ),
...
CREATE TABLE IF NOT EXISTS round_stats (
    ...
    team_1_buy_type TEXT NOT NULL CHECK (
        team_1_buy_type IN (
            'pistol',
            'eco',
            'semi_eco',
    ...
    team_2_buy_type TEXT NOT NULL CHECK (
        team_2_buy_type IN (
            'pistol',
            'eco',
            'semi_eco',
    ...
    won_method TEXT NOT NULL CHECK (
        won_method IN (
            'eliminate',
            'spike_explode',
            'defuse',
...
CREATE TABLE IF NOT EXISTS player_overview_stats (
    ...
    kast REAL NOT NULL CHECK (
        kast >= 0
        AND kast <= 100
    ),
    adr REAL NOT NULL CHECK (adr >= 0),
    hs REAL NOT NULL CHECK (
        hs >= 0
        AND hs <= 100
    ),
    first_kills INTEGER NOT NULL CHECK (first_kills >= 0),
    first_deaths INTEGER NOT NULL CHECK (first_deaths >= 0),
    FOREIGN KEY (match_id) REFERENCES matches (id),
    FOREIGN KEY (map_id) REFERENCES maps (id),
...
CREATE TABLE IF NOT EXISTS ban_pick_log (
    ...
    map_id INTEGER NOT NULL,
    veto_action TEXT NOT NULL CHECK (veto_action IN ('ban', 'pick', 'remain')),
...
CREATE TABLE IF NOT EXISTS teams (
    ...
    country_id INTEGER,
    region_id INTEGER,
    FOREIGN KEY (country_id) REFERENCES countries (id) FOREIGN KEY (region_id) REFERENCES regions (id)
);
...
CREATE TABLE IF NOT EXISTS regions (id INTEGER PRIMARY KEY, name TEXT UNIQUE NOT NULL);
...
CREATE TABLE IF NOT EXISTS agents (
    ...
    agent_type TEXT NOT NULL CHECK (
        agent_type IN ('duelist', 'controller', 'sentinel', 'initiator')
    ),
    release_date TEXT NOT NULL
);
...
INSERT OR IGNORE INTO
    maps_pool (date, patch_no, map_id)
VALUES
...
INSERT OR IGNORE INTO
    agents (name, agent_type, release_date)
VALUES
